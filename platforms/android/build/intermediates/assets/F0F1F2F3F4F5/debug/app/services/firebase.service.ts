...
import { User, Gift, UserName} from "../models";
...
export class FirebaseService {
  ...
  private _allusersnames: Array<UserName> = [];
 ...
 this.isloggedin.next(false);
  ...
  handleUserNamesSnapshot(data: any) {
      //empty array, then refill and filter
      this._allusersnames = [];
      if (data) {
          for (let id in data) {
              let result = (<any>Object).assign({ id: id }, data[id]);
              if (BackendService.email != result.email) {
  ...
  createuserrecord(user: User) {
      ...
      return firebase.push(
          "/RegisteredUsers",
          { "email": user.email,  "date": 0 - Date.now() }
      ).then(
          function (result: any) {
  ...
  getUserList(): Observable<any> {
      return new Observable((observer: any) => {
          ...
          let onValueEvent = (snapshot: any) => {
              this.ngZone.run(() => {
                  let results = this.handleUserNamesSnapshot(snapshot.value);
                  console.log(JSON.stringify(results))                 
                  observer.next(results);
              });
          };
